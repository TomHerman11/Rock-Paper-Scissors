...
void Manager::writetoOutputFile(int winning_cause, int winner) {
	string reason;
	Piece piece;
	RPS_Point tmp_point;
	char pieceType;
...
void Manager::increaseGameFieldsUponMoving(const Piece &tmpPiece) {
...
void Manager::reduceGameFieldsUponMoving(const Piece &piece) {
...
void Manager::movePiece(Piece &piece, const Point &point, unique_ptr<FightInfo>& unq_ptr) {
...
void Manager::insertPositionPieces(	int player,
...
int Manager::checkWinnerAfterPosition(){
	if (flagsPlayer1==0 && flagsPlayer2==0) { //tie
		writetoOutputFile(TIE_FLAGS_EATEN, 0);
		return 1;
	}

	if (movingPiecesPlayer1==0  && movingPiecesPlayer2==0) { //tie
		writetoOutputFile(TIE_NO_MOVING_PIECES, 0);
		return 1;
	}

	//player 1 loses:
	if (flagsPlayer1==0) {
		writetoOutputFile(PLAYER_FLAGS_EATEN, 2);
		return 1;
	}

	if (movingPiecesPlayer1==0) {
		writetoOutputFile(PLAYER_NO_MOVING_PIECES, 2);
		return 1;
	}

	//player2 loses
	if (flagsPlayer2==0) {
		writetoOutputFile(PLAYER_FLAGS_EATEN, 1);
		return 1;
	}
	if (movingPiecesPlayer2==0) {
		writetoOutputFile(PLAYER_NO_MOVING_PIECES, 1);
		return 1;
	}
	return 0; //no winner so far
}

void Manager::changeJokerRep(const JokerChange& jokerChange){
...
int Manager::checkLegalMove(const Move& move, int player) {
...
int Manager::checkLegalJokerChange(const JokerChange& jokerChange, int player) {
	//check if in board limits
...
int Manager::playerHasMovingPieces(int player){
	int result;
	if(player==1){
...
int Manager::playerHasFlag(int player){
	int result;
	if(player==1){
...
int Manager::checkWinner(int player){
	int opponent= (player==1)? 2:1;
	if(playerHasFlag(opponent)==0){
		return 1;// opponent has no flags
	}
	if(playerHasMovingPieces(opponent)==0){
		return 2;// opponent has no moving pieces
	}
	return 0;
}

int Manager::game_manager(PlayerAlgorithm& player1, PlayerAlgorithm& player2){
	int legal_pos1;
	int legal_pos2;
	int noFightMoves=0;
	//int sumFights=0;
