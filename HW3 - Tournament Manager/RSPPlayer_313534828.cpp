/*
 * RSPPlayer_313534828.cpp
 *
 *  Created on: May 7, 2018
 *      Author: tomhe
 */

#include "RSPPlayer_313534828.h"

REGISTER_ALGORITHM(313534828)
...
void RSPPlayer_313534828::findRandomClearPosition(RPS_Point& point) {
...
void RSPPlayer_313534828::updateBoardandVecotr(	int player,
												...
												char piece_type)
{
	...
	if (piece_type=='J') {
		is_joker=1;
		type_for_inside_board='R';
		joker_rep='R'; //TO BE CHANGED LATER?
	}
	RPS_Point tmp_point;
	Piece tmp_piece;
	...
	for (int i=0; i<number_of_pieces; i++) {
		...
		vectorToFill.push_back(std::make_unique<RPS_PiecePosition>(RPS_PiecePosition(piece_type,
																				joker_rep,
																				tmp_point.getX(),
																				tmp_point.getY())));
	}
...
int RSPPlayer_313534828::getMovefindToPoint(const RPS_Point& from_point, RPS_Point& to_point) {
...
int RSPPlayer_313534828::getMovefindOffensiveToPoint(const RPS_Point& from_point, RPS_Point& to_point) {
	int from_col=from_point.getX();
	int from_row=from_point.getY();
...
void RSPPlayer_313534828::getInitialPositions(	int player,
												std::vector<unique_ptr<PiecePosition>>& vectorToFill)
{
...
void RSPPlayer_313534828::notifyOnInitialBoard(const Board& b, const std::vector<unique_ptr<FightInfo>>& fights){
	...
	//update moving pieces and jokers alive fields
	this->moving_pieces=0;
	this->jokers_alive=0;

	for(int i=1; i<=M_COLUMNS; i++){ //columns
		for(int j=1; j<=N_ROWS; j++){//rows
			pos= RPS_Point(i,j);
			if(board.getPlayer(pos)==player){
				Piece& myPiece=board.getPiece(pos);
				if (myPiece.isMovingPiece()) {
					this->moving_pieces++;
				}
				if (myPiece.getIsJoker()) {
					this->jokers_alive++;
				}
			}
		}
...
void RSPPlayer_313534828::notifyOnOpponentMove(const Move& move){
...
void RSPPlayer_313534828::notifyFightResult(const FightInfo& fightInfo){
	...
	//update moving_pieces and jokers_alive:
	if (winner!=player) {
		if (last_move_piece.isMovingPiece()) {
			moving_pieces--;
		}
		if (last_move_piece.getIsJoker()) {
			jokers_alive--;
		}
	}
}

unique_ptr<Move> RSPPlayer_313534828::getMove() {
	...
	if(move_found){
		ptr=std::make_unique<RPS_Move>(RPS_Move(from_point, to_point));
	...
	ptr=std::make_unique<RPS_Move>(RPS_Move(-1,-1,-1,-1));
	return ptr;
}

unique_ptr<JokerChange> RSPPlayer_313534828::getJokerChange() {
	...
	if (this->jokers_alive==0) {
		return ptr;
	}

	for (int i=1; i<=N_ROWS; i++) {
		for (int j=1; j<=M_COLUMNS; j++) {
			tmp_point=RPS_Point(j,i);
			Piece& tmp_piece=board.getPiece(tmp_point);
			if (tmp_piece.getOwnerPlayer()==player && tmp_piece.getIsJoker()) {
				from_col=j;
				from_row=i;
				player_type=tmp_piece.getType();

				RPS_Point possible_points[]={RPS_Point(from_col, from_row+1), RPS_Point(from_col, from_row-1), RPS_Point(from_col+1, from_row), RPS_Point(from_col-1, from_row)};
				for (int k=0; k<4; k++) {
					if (board.getPlayer(possible_points[k])==opponent && validPoistiononBoard(possible_points[k])==1) {
						opponent_type=board.getPiece(possible_points[k]).getType();
						if (opponent_type=='R') {new_type='P'; break;}
						if (opponent_type=='P') {new_type='S'; break;}
						if (opponent_type=='S') {new_type='R'; break;}
					}
				}
				if (new_type!=0) {
					if (player_type!=new_type) {
						//will change the joker rep:
						if (player_type=='B') {moving_pieces++;}
						ptr=std::make_unique<RPS_JokerChange>(RPS_JokerChange(from_col, from_row, new_type));
						return ptr;
					}
					else {
						new_type=0; //for next joker.
					}
				}
			}
		}
	...
	if (moving_pieces==0) { //we have to change a joker rep o/w will lose (and we know joker_alive>0)
		ptr=std::make_unique<RPS_JokerChange>(RPS_JokerChange(from_col, from_row, 'R'));
		return ptr;
	}
